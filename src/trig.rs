...
/// Map sin to each element of an array.
pub fn sin_map<const N: usize>(thetas: [f32; N]) -> [f32; N] {
    let mut sines: [f32; N] = [0.; N];
    for n in 0..N {
        sines[n] = sin(thetas[n]);
    }
...
/// Map cos to each element of an array.
pub fn cos_map<const N: usize>(thetas: [f32; N]) -> [f32; N] {
    let mut cosines: [f32; N] = [0.; N];
    for n in 0..N {
        cosines[n] = cos(thetas[n]);
    }
